...
**Karaoke video processing pipeline (v2 feature-complete, awaiting promotion)**: `karaoke-processing/karaoke-process-v2` adds four new options on top of the v1 luminance-LUT pipeline: `-splash SECONDS` (preserve intro splash unaltered), `-z PERCENT` (uniform centered zoom for readability), `--invert-bands` (swap mid/high LUT bands — rescues Party Tyme and APT-style channels where unsung text is brighter than sung text), and `--outline N` (high-contrast two-ring gray halo for FCP overlay readability; default 2; 0 disables). All four options validated end-to-end on ROSÉ & Bruno Mars - APT. Next: validate v2 across more channels, then promote v2 → v1 by replacing `~/.local/bin/karaoke-process`.
